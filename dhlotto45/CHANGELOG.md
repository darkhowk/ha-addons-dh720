...
### 🐛 수정
- **MQTT Discovery 경고 해결**
  - `object_id` deprecated → `default_entity_id` 사용
  - Home Assistant Core 2026.4 호환성 확보
  
- **Timestamp timezone 오류 해결**
  - `datetime.now().isoformat()` → `datetime.now(timezone.utc).isoformat()`
  - sensor.lotto45_last_update 정상 작동

### 📚 문서
- FAQ.md 추가: 자주 묻는 질문 10가지
  - Q1: 구매 버튼이 없는 이유
  - Q2: MQTT 경고 해결
  - Q3: 통합구성요소 vs 애드온
  - Q4~Q10: 기타 문제 해결
