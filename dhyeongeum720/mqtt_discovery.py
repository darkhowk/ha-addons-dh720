...
async def publish_button_mqtt(
    ...
    """
    Helper function to publish button via MQTT Discovery

    Args:
        mqtt_client: MQTT Discovery client
        entity_id: Entity ID (e.g., 'pension720_buy_1')
        username: DH Lottery username
        name: Friendly name (optional, defaults to entity_id)
        icon: Icon (optional, defaults to 'mdi:gesture-tap-button')
    ...
    # Prepare command topic
    command_topic = f"homeassistant/button/{TOPIC_PREFIX}_{username}_{entity_id}/command"

    # Publish discovery config
    result = mqtt_client.publish_button_discovery(
        button_id=entity_id,
        name=name,
        command_topic=command_topic,
        username=username,
    ...
    if result:
        _LOGGER.info(f"MQTT button published: button.{TOPIC_PREFIX}_{username}_{entity_id}")
